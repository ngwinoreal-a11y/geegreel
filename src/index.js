...
async function handle(request, env, ctx) {
  ...
  if (liveEndMatch && method === "POST") {
    requireUser(user);
    const session = await env.DB.prepare("SELECT * FROM live_sessions WHERE id = ?").bind(liveEndMatch[1]).first();
    if (!session) return err("Live not found", 404);
    if (session.host_user_id !== user.id) return err("That isn't your live", 403);

    await env.DB.prepare("UPDATE live_sessions SET status='ended', ended_at=? WHERE id=?")
      .bind(now(), session.id).run();

    const room = env.LIVE_ROOMS.get(env.LIVE_ROOMS.idFromName(session.id));
    await room.fetch("https://do/broadcast", {
      method: "POST", body: JSON.stringify({ type: "ended" }),
    });
    return json({ ok: true });
  }
  ...
  if (path === "/api/live/active" && method === "GET") {
    const { results } = await env.DB.prepare(`
      SELECT ls.id, ls.title, ls.started_at,
             u.id AS host_id, u.username, u.display_name, u.avatar_key
      FROM live_sessions ls JOIN users u ON u.id = ls.host_user_id
      WHERE ls.status = 'live' AND u.status != 'banned'
      ORDER BY ls.started_at DESC LIMIT 50
    `).all();
    return json({
      sessions: results.map(s => ({
        id: s.id, title: s.title, startedAt: s.started_at,
        host: {
          id: s.host_id, username: s.username, displayName: s.display_name,
          avatarUrl: s.avatar_key ? `/api/media/${s.avatar_key}` : null,
        },
      })),
    });
  }

  const liveGetMatch = /^\/api\/live\/([\w-]+)$/.exec(path);
  if (liveGetMatch && method === "GET") {
    const s = await env.DB.prepare(`
      SELECT ls.id, ls.title, ls.status, ls.started_at, ls.ended_at,
             u.id AS host_id, u.username, u.display_name, u.avatar_key
      FROM live_sessions ls JOIN users u ON u.id = ls.host_user_id
      WHERE ls.id = ?
    `).bind(liveGetMatch[1]).first();
    if (!s) return err("Live not found", 404);
    return json({
      session: {
        id: s.id, title: s.title, status: s.status, startedAt: s.started_at, endedAt: s.ended_at,
        host: {
          id: s.host_id, username: s.username, displayName: s.display_name,
          avatarUrl: s.avatar_key ? `/api/media/${s.avatar_key}` : null,
        },
  ...
  // WebSocket signaling/chat/gift-relay channel. A browser's WebSocket
  // constructor can't send an Authorization header, so the session token
  // travels as ?token= instead — see getUserByToken's comment.
  const liveSocketMatch = /^\/api\/live\/([\w-]+)\/socket$/.exec(path);
  if (liveSocketMatch) {
    if (request.headers.get("Upgrade") !== "websocket") return err("Expected a WebSocket", 426);
    const wsUser = await getUserByToken(env, url.searchParams.get("token"));
    if (!wsUser) return err("Not authenticated", 401);

    const session = await env.DB.prepare("SELECT * FROM live_sessions WHERE id = ?").bind(liveSocketMatch[1]).first();
    if (!session || session.status !== "live") return err("This live has ended", 404);

    const forwardUrl = new URL(request.url);
    forwardUrl.searchParams.set("uid", wsUser.id);
    forwardUrl.searchParams.set("uname", wsUser.username);
    forwardUrl.searchParams.set("udisplay", wsUser.display_name || "");
    forwardUrl.searchParams.set("uavatar", wsUser.avatar_key || "");
    forwardUrl.searchParams.set("ishost", wsUser.id === session.host_user_id ? "1" : "0");

    const room = env.LIVE_ROOMS.get(env.LIVE_ROOMS.idFromName(session.id));
    return room.fetch(new Request(forwardUrl, request));
  }

  // A gift sent during a live. Same balance-safe pattern as /api/gifts (the
  // WHERE clause is the actual guard, not a JS check beforehand) — kept as
  // its own endpoint rather than reusing /api/gifts because a live session
  // isn't a video, and this also has to notify LiveRoom afterward so the
  // gift animation shows for everyone watching right now.
  const liveGiftMatch = /^\/api\/live\/([\w-]+)\/gift$/.exec(path);
  if (liveGiftMatch && method === "POST") {
    requireUser(user);
    const { giftKey } = await request.json();
    const gift = giftByKey(giftKey);
    if (!gift) return err("Unknown gift");

    const session = await env.DB.prepare(`
      SELECT ls.*, u.status AS host_status FROM live_sessions ls
      JOIN users u ON u.id = ls.host_user_id WHERE ls.id = ?
    `).bind(liveGiftMatch[1]).first();
    if (!session || session.status !== "live") return err("This live has ended", 404);
    if (session.host_status === "banned") return err("That account is banned", 403);
    if (session.host_user_id === user.id) return err("You can't gift your own live");

    const giftId = uid();
    const creatorCents = Math.floor(gift.coins * COIN_PRICE_CENTS * CREATOR_SHARE);

    const spend = await env.DB.prepare(
      "UPDATE users SET coin_balance = coin_balance - ? WHERE id = ? AND coin_balance >= ?"
    ).bind(gift.coins, user.id, gift.coins).run();
    if (spend.meta.changes === 0) return err("Not enough coins", 402);

    await env.DB.batch([
      env.DB.prepare(`
        INSERT INTO live_gifts (id, sender_id, recipient_id, session_id, gift_key, coins, value_cents, created_at)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?)
      `).bind(giftId, user.id, session.host_user_id, session.id, gift.key, gift.coins, creatorCents, now()),
      env.DB.prepare(`
        UPDATE users SET gift_balance_cents = gift_balance_cents + ?,
                         lifetime_gifts_cents = lifetime_gifts_cents + ?
        WHERE id = ?
      `).bind(creatorCents, creatorCents, session.host_user_id),
    ]);

    const [me, them] = await Promise.all([
      env.DB.prepare("SELECT coin_balance, gift_balance_cents FROM users WHERE id = ?").bind(user.id).first(),
      env.DB.prepare("SELECT coin_balance, gift_balance_cents FROM users WHERE id = ?").bind(session.host_user_id).first(),
    ]);
    await recordTx(env, user.id, "gift_sent", -gift.coins, 0,
                   me.coin_balance, me.gift_balance_cents, giftId, `${gift.emoji} ${gift.name} (live)`);
    await recordTx(env, session.host_user_id, "gift_received", 0, creatorCents,
                   them.coin_balance, them.gift_balance_cents, giftId, `${gift.emoji} ${gift.name} (live)`);
    ...
    await room.fetch("https://do/broadcast", {
      ...
      body: JSON.stringify({
        type: "gift",
        user: { id: user.id, username: user.username, displayName: user.display_name, avatarUrl: user.avatar_key ? `/api/media/${user.avatar_key}` : null },
        gift: { key: gift.key, name: gift.name, emoji: gift.emoji, coins: gift.coins },
      }),
    });

    return json({ ok: true, coinsLeft: me.coin_balance }, 201);
  }

  // The host uploads short rolling clips of the composited "stage" (their
  // own camera plus any approved guests) as it records them — this is how
  // viewers watch: no per-viewer WebRTC connection, they just fetch and play
  // these clips in order a couple of seconds behind real time. Scales to any
  // number of viewers on plain R2 reads instead of needing an SFU.
  const liveChunkMatch = /^\/api\/live\/([\w-]+)\/chunk$/.exec(path);
  if (liveChunkMatch && method === "POST") {
    requireUser(user);
...
// ============================================================================
// LiveRoom — one Durable Object instance per live session, holding its
// WebSocket connections. This is purely a real-time relay: it forwards
// WebRTC signaling (offer/answer/ICE) between specific peers, relays chat and
// gift-arrived events to everyone connected, and tells the host about
// join requests. It never touches coins or the database directly for
// anything money-related — /api/live/:id/gift in handle() above does the
// actual balance-safe transaction and then calls this object's /broadcast
// endpoint to announce it, the same separation the rest of the app already
// keeps between "what changed the balance" and "what tells the UI about it".
//
// Uses the Hibernatable WebSockets API (acceptWebSocket/serializeAttachment)
// so the object doesn't stay billed as active between messages — it can be
// evicted from memory and woken back up by the runtime without losing track
// of who's connected, since each socket's identity travels with it via
// serializeAttachment rather than living in a plain in-memory Map.
export class LiveRoom {
  ...
  publicInfo(info) {
    return {
      ...
      avatarUrl: info.avatarKey ? `/api/media/${info.avatarKey}` : null,
      role: info.role,
    };
  }
  ...
  broadcastViewerCount() {
    this.broadcast({ type: "viewerCount", count: this.state.getWebSockets().length });
  }

  async fetch(request) {
    const url = new URL(request.url);

    // Internal, server-to-object calls from handle() above — never reached
    // directly by a client. Used for events that must be validated and
    // committed to D1 first (ending a live, a gift's coin transfer) and only
    // then announced to whoever's connected.
    if (url.pathname === "/broadcast" && request.method === "POST") {
      const msg = await request.json();
      this.broadcast(msg);
      if (msg.type === "ended") {
        for (const ws of this.state.getWebSockets()) {
          try { ws.close(1000, "Live ended"); } catch {}
        }
      }
    ...
    const info = {
      ...
      displayName: url.searchParams.get("udisplay") || "",
      avatarKey: url.searchParams.get("uavatar") || "",
      isHost: url.searchParams.get("ishost") === "1",
      role: url.searchParams.get("ishost") === "1" ? "host" : "viewer",
    };
    ...
    server.send(JSON.stringify({
      type: "welcome",
      you: this.publicInfo(info),
      participants: others.map(s => this.publicInfo(s.deserializeAttachment())),
    }));
    this.broadcast({ type: "joined", user: this.publicInfo(info) }, server);
  ...
  async webSocketMessage(ws, message) {
    ...
    if (msg.type === "approve" || msg.type === "decline") {
      if (!info.isHost) return; // only the host's own connection can approve
...
export default {
  async fetch(request, env, ctx) {
    ...
    try {
      ...
      if (res) {
        // A WebSocket upgrade (status 101, carrying a .webSocket pair) is not
        // an ordinary response — rebuilding it via `new Response(res.body, res)`
        // to patch in a CORS header would drop that pairing and break the
        // handshake, and CORS headers are meaningless on a 101 anyway.
